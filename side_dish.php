...
<head>
    ...
    <title>Student Recipe | Side Dish</title>
</head>
<body>
<?php
  include 'header.php';
...
<?php
    $id = $_GET["id"];
    $side = "SELECT picture, name, prep_time, cook_time, ingredient, steps, video_url, videoname FROM recipes WHERE $id = id AND category='side_dish'";
    $result = mysqli_query($conn, $side);
